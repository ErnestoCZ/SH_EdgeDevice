...
static const struct gpio_dt_spec led_adv_status =
    GPIO_DT_SPEC_GET(STATUS_LED_ADV_NODE, gpios);
static const struct gpio_dt_spec led_device_status =
...
int init_gpio(void) {
  ...
  if (gpio_is_ready_dt(&button3)) {
    ...
    ret = gpio_pin_interrupt_configure_dt(&button3, GPIO_INT_EDGE_FALLING);
